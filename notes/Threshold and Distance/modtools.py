...
def getFunStochThreshold(fr, threshold_array, escape_func = None, return_slice=False, dt=1, dx=1):
    """
    Args:
        fr (3d array): has the format (trials, space, time)
        threshold_array (array): used the function getArryFromMath(),
    ...
    if escape_func is None:
        tau = 0.5; beta = 10. # see plot above, in real neurons it can be 19 ms and 4 mV
        escape_func = lambda x: 1 / tau * np.exp(beta * x)
    distance = fr - threshold_array
    escape_prob = 1 - np.exp(-dt*escape_func(distance))
    triggered = np.random.uniform(low=0.0, high=1.0, size=escape_prob.shape) < escape_prob

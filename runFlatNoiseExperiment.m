...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rall]= runFlatNoiseExperiment(soa,no_trials,noise_amplitude)
...
A=40;
I=55;
sig_w = 14.0; % 0.7 * mm_to_radian;  % we simulate 10 mm of SC with sig=0.7mm
...
aendo_targ = 0; aendo_targ_gap = 0; aexo_targ = 0;
...
I_fix =  stepFunction(time, aendo_fix,  fix_on  + dendo, fix_off  + dendo)...
    + expDecrease(time, aexo_fix, fix_on + dexo, tau_on);
I_targ = stepFunction(time, aendo_targ, targ_on, targ_off); % not used!
I_dist = stepFunction(time, aendo_dist, dist_on + dendo, dist_off + dendo); % not used!
